...
const productPreloader = document.querySelector('#holder');
...
function createButton(buttonElement, classname, activeClass, isActive) {
    function toggleActiveClass(element) {
        productPreloader.style.display = "block";
        ...
        if (element.classList.contains('buttons__category')) {
            let activeCats = [];
            for (let el = 0; el < elements.length; el++) {
                if(elements[el].classList.contains(activeClass)){
                    activeCats.push(elements[el])
                }
            }
        ...
        if (initClass.includes('buttons__filter')) {
            ...
            let isAllSubcats = element.value==1? true : false;
            // Если категория все
            if(isAllSubcats){
                ...
                if (toggleAdded) {
                    for (let el = 0; el < elements.length; el++) {
                        elements[el].classList.add(activeClass);
...
async function renderProducts(subcats, products, parentContainer, activeCat = undefined) {
        ...
        for (let s = 0; s < subcats.length; s++) {
            for (let p = 0; p < products.length; p++) {
                if (products[p].subcat && Array.isArray(products[p].subcat) && products[p].subcat.includes(subcats[s].id)) {
                    ...
                    productCard.classList.add('active');
                    if(productsArray.includes(products[p])){

                    }
                    else{
                        productCards.push(productCard);
                    }
                    productsArray.push(products[p]);
                }
            }
        }
    }
    ...
    productPreloader.style.display = "none";
}
...
function returnActiveElements(catId, subcatId, data) {
    ...
    if (data && data.subcats && Array.isArray(data.subcats) && Array.isArray(subcatId)) {
        if(subcatId.includes('1')){
            activeSubcat.push({id: 1, title:"Все категории"});
        }
...
function renderSubcats(subcats, actives, parentContainer, activeCat) {
    ...
    for(let l=0; l<actives.length; l++){
        if(actives[l].id=='1'){
            isAllActive=true;
            break;
        }
    ...
    if(isAllActive){
         subcatsContainer.appendChild(createButton({id:1, title:"Все категории"}, FILTER_BTN, ACTIVE_SUBCAT,true));
    }
    else{
        subcatsContainer.appendChild(createButton({id:1, title:"Все категории"}, FILTER_BTN, ACTIVE_SUBCAT));
    }
